...
Class Action {
	...
	function save_fees(){
		...
		extract($_POST);
		$data = "";
		$client =  $this->db->query("SELECT name, id_no FROM student WHERE id=". $_POST["student_id"])->fetch_assoc();
		$property = $this->db->query("SELECT course  FROM courses WHERE id=". $_POST["course_id"])->fetch_assoc();
		foreach($_POST as $k => $v){
			if(!in_array($k, array('id')) && !is_numeric($k)){
				if($k == 'total_fee'){
					$v = str_replace(',', '', $v);
		...
		$identifier = $client["id_no"]."|".$property["course"];
		$data .= ", ef_no='".$identifier."'";
		$check = $this->db->query("SELECT * FROM student_ef_list where ef_no ='$identifier' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
	...
	function save_payment(){
		...
		// if(!empty($id)){
		// 	$amountPaid = $this->db->query("SELECT amount FROM payments WHERE id = $id")->fetch_assoc()['amount'] ?? 0;
		...
		if(empty($id)){
			...
			$paid_fees=$paid_fees + 1;
			$data .= ", fee_no='$paid_fees'";
			if($paid_fees <= $total_fees) {
				
				$save = $this->db->query("INSERT INTO payments set $data");
				
				if($save)
					$this->db->query("UPDATE student_ef_list SET paid_fees = $paid_fees where id= $ef_id");
					$id= $this->db->insert_id;
			}else
			{
				return 2;
	...
	function update_exchange_rate(){
		...
		$check = $this->db->query("SELECT exchange_rate FROM configuration where id = 1 ")->num_rows;
		if($check < 1){
			return 2;
			exit;
		}
		
		$save = $this->db->query("UPDATE configuration set $data where id = 1");
